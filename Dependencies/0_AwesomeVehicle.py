'''
The code is a plug-in hybrid vehicle simulator, that run a complete New European Drive Cycyle or other drive cycles. The code works in collabration with a NN that gives the
optimal shift signals to the transimission to run the vehicle in. The code is a part of my master thesis and was carried out in CEVT AB. The data used is the integral part of the compnay and hence has been
replaced here, so that the data rights are not violated here :)
 Curious?
...
class PHEV(object):
	...
	def __init__(self,velProfileFile,KerasModel,config={}):
		...
		config.setdefault('battery_capacity',15) # 17 kilo watt hour battery
	...
	def UpdateCycleDynamics(self):
		...
		RLC = lambda v: 100 + 0.1*v + 0.01*v**2 # depends on mass
	...
	def RunSimulation(self):
		...
		listSignalsRequired = ['engGear','emGear','soc','normaLizedSOC','emShiftGear','engShiftGear','emRPM',
								...
								'normalizedSOCLimit','normalizedSOCTarget','mode','elecCurrent','powerMech','powerEm','engPower','energyEm','powerMotor','tEff']
		...
		try:
			while self.vehSpeed[self.ii]>=0 and self.ii<nPoints-1:
				...
				#print('The demand Torque in Run sim  is',self.demTrq[self.ii])
				self.normalizedSOCLimit[self.ii-1] = self.soc[self.ii-1]/self.SOC_min
				self.normalizedSOCTarget[self.ii-1] = self.soc[self.ii-1]/self.SOC_target
				inputDataNN = np.array([self.normalizedSOCLimit[self.ii-1],self.normalizedSOCTarget[self.ii-1],self.whlSpeed[self.ii]/10,self.demTrq[self.ii]/150,self.emGear[self.ii-1],self.engGear[self.ii-1]]).reshape((1,self.nInputs)).copy()
				self.outputNN[self.ii,:] = np.array(self.ControllerNN.predict(inputDataNN)).ravel()
				emShiftGear, engShiftGear, mode, alpha, beta = self.OutputNNToGear(self.outputNN[self.ii,:])
				self.alpha[self.ii] = alpha
				#print('The alpha value is:',alpha)
				self.beta[self.ii] = beta
				#mode = 1
				if (self.vehSpeed[self.ii]==0): # talk to borja 
					self.engGear[self.ii] = self.initEngGear
					self.emGear[self.ii] = self.initEmGear
					self.UpdateGearArray(engShiftGear,emShiftGear)
					#print(engShiftGear,'and',emShiftGear)
				self.Engine_Mode(0)
				self.EV_Mode(0)
				self.UpdateGearArray(engShiftGear,emShiftGear)
	...
	def CheckConstraints(self):
		if self.soc[self.ii]< self.SOC_min:# or self.emRPM[self.ii] >= 12000 or self.engRPM[self.ii] >= 4000 or self.engTorque[self.ii] >= 160 or self.emTorque[self.ii] >= 165:
			sys.exit()
		pass
	...
	def Engine_Mode(self,demTrq): #without charging the battery
		...
		if demTrq > 0:
			...
			self.fuelFlow[self.ii] = self.engPower[self.ii]/self.engEff[self.ii]/self.Q_heat # + idle fuel flow l/sec
		else:
			self.engTorque[self.ii] = self.engTorque[self.ii]*self.tEff[self.ii]
			self.EV_Mode(demTrq)
			'''if demTrq>0:
