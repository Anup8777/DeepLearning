...
The code is a conventional vehicle simulator, that run a complete New European Drive Cycyle or other drive cycles. The code works in collabration with a NN that gives 
...
class ICE(object):
	...
	def RunSimulation(self):
		...
		listSignalsRequired = ['engGear','emGear','soc','normaLizedSOC','emShiftGear','engShiftGear','emRPM',
								...
								'normalizedSOCLimit','normalizedSOCTarget','mode','elecCurrent','powerMech','powerMotor','engPower','energyEm','tEff']
		...
		while self.vehSpeed[self.ii]>=0 and self.ii<nPoints-1:
			self.ii += 1
			'''if (self.vehSpeed[self.ii]==0):
				self.engGear[self.ii] = self.initEngGear
				self.emGear[self.ii] = self.initEmGear'''
			_,_,gearOpt,_,_ = UT.GetOptimalGear(self.whlSpeed[self.ii]*60,self.demTrq[self.ii],self.engGearRatios,self.EngineMap)
			if gearOpt[0]>self.engGear[self.ii-1]:
				engShiftGear = 1
			elif gearOpt[0] == self.engGear[self.ii-1]:
				engShiftGear = 0
			else:
				engShiftGear = -1
			mode = 1
	...
	def EV_Mode(self,demTrq):
		...
		self.powerMech[self.ii] += 2*np.pi*self.emRPM[self.ii]*self.emTorque[self.ii]/60
	...
	def Engine_Mode(self,demTrq): #without charging the battery
		...
		self.engRPM[self.ii] = np.maximum(self.whlSpeed[self.ii]*self.engGearRatios[int(self.engGear[self.ii]-1)]*60,900)
		...
		if demTrq > 0:
			...
			demTrq =0
			self.engTorque[self.ii] = 0
			self.engRPM[self.ii] = 0
	...
	# def Engine_Mode(self,demTrq): #without charging the battery
		...
		# if demTrq>0:
			...
			# self.powerMech[self.ii] += 2*np.pi*self.whlSpeed[self.ii]*demTrq
			# self.fuelFlow[self.ii] = self.engPower[self.ii]/self.engEff[self.ii]/self.Q_heat
